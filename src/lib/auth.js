...
export const strapiRegister = (userData) => {
  if (!process.browser) {
    return undefined;
  }
  // console.log(username, email, password);
  strapi.register(userData.username, userData.email, userData.password, userData.organization).then(res => {
    setToken(res);
...
export const strapiLogin = (userData) => {
  ...
  // Get a token
  return strapi.login(userData.email, userData.password).then(res => {
    setToken(res);
